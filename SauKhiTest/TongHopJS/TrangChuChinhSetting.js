...
(function ($) {
  ...
  // Tạo khung chỉnh mặc định (Update 11/07/2020 21:14)
  $Khung.append("<center><h6>Chỉnh Mặc Định</h6></center>");

  var $SideDenChinh = $("<div />", {
    class: "d-flex",
  });
  $Khung.append($SideDenChinh); // Đưa Khung Chỉnh Mặc Định Vào

  var $btn_MacDinh = $("<a />", {
    //tạo thẻ a bằng jquery
    href: "../TrangChuChinh.php",
  }) //cài đặt css và các sự kiện
    .text("Mặc Định")
    .css({
      position: "absolute",
      left: "36%",
    ...
    .on("click", function (e) {
      ...
      //remove màu của side
      var sideremove = "sidebar-dark-blue-grey-darken-4";
      var $LuongSide = $(".main-sidebar");
      sidebar_skins.forEach(function (skin) {
        $LuongSide.removeClass(skin);
      });
      $LuongSide.addClass(sideremove);

      //remove màu của nav
      var navremove = "navbar-light";
      var $main_header = $(".main-header");
      $main_header.removeClass("navbar-dark").removeClass("navbar-light");
      navbar_all_colors.forEach(function (color) {
        $main_header.removeClass(color);
      });
      $main_header.addClass(navremove);
    });

  $Khung.append($btn_MacDinh);
  ...
  function createSkinBlock(colors, callback) {
    ...
    colors.forEach(function (color) {
      ...
      $color.hover(
        // tạo hiệu ứng di chuột vào đối tượng trong jquery
        function () {
          $(this)
            .css({ opacity: 1 })
